...
- Added JPEG Ultra HDR / JPEG_R image preview generation. For supported JPEG
  ...
  with gain maps and stores them as HDR-specific preview variants. Attachments
  with generated HDR previews expose `meta.hdr: true`, and clients can request
  the HDR version with `GET /v4/attachments/:id/image?hdr`. The `hdr` query
  parameter is safe to send for any image attachment: if an HDR preview is not
  available, the endpoint falls back to the regular SDR preview.
