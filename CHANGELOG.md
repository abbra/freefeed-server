...
- The ability to undo actions.
  - The destructive actions (like post  or comment deletion) now returns an
    ...
    following shape: `{ subject: string; message: string; token: string;
    expiresInSec: number; extra?: object }`. The undo token is valid for
  ...
  - For now, only the post and comment deletion can be undone, but the plan is
    to add more.
